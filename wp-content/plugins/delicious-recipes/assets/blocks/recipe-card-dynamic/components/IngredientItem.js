...
export default class IngredientItem extends Component {
	...
	getButtons() {
		...
		return (
			<div className="ingredient-item-button-container">
				{this.getMover()}
				{/* {!isGroup && (
					<MediaUpload
		...
		return (
			<Fragment>
				<Button
					...
					icon="arrow-up-alt2"
					label={__("Move item up", "delicious-recipes")}
				...
				<Button
					...
					label={__("Move item down", "delicious-recipes")}
	...
	render() {
		const {
			isSelected,
			subElement,
			item: { id, name, isGroup },
		} = this.props;

		const isSelectedName = isSelected && subElement === "name";
		const itemClassName = !isGroup
			? "ingredient-item"
			: "ingredient-item ingredient-item-group";
