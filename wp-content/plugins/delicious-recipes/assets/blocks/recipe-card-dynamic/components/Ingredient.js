...
export default class Ingredient extends Component {
	...
	constructor(props) {
		...
		this.onAddIngredientButtonClick = this.onAddIngredientButtonClick.bind(
			this
		);
	...
	changeItem(newName, previousName, index, group = false) {
		const ingredients = this.props.attributes.ingredients
			? this.props.attributes.ingredients.slice()
			: [];
	...
	insertItem(index = null, name = [], focus = true, group = false) {
		...
		while (lastIndex > index) {
			this.editorRefs[`${lastIndex + 1}:name`] = this.editorRefs[
				`${lastIndex}:name`
			];
			lastIndex--;
		}
		...
		speak(__("New ingredient added", "delicious-recipes"));
	}
	...
	removeItem(index) {
		...
		while (this.editorRefs[`${nextIndex}:name`]) {
			this.editorRefs[`${nextIndex - 1}:name`] = this.editorRefs[
				`${nextIndex}:name`
			];
			nextIndex++;
		}
		...
		speak(__("Ingredient removed", "delicious-recipes"));
	}
	...
	onAddGroupButtonClick() {
		...
		focusIndex =
			focusIndex != "" && focusIndex != "ingredientsTitle"
				? toNumber(focusIndex)
				: null;
		...
		return this.props.attributes.ingredients.map((item, index) => {
			return (
				<IngredientItem
					...
					isFirst={index === 0}
					isLast={
						index === this.props.attributes.ingredients.length - 1
					}
	...
	getAddItemButton() {
		return (
			<div className="ingredients-add-buttons">
				...
				>
					<span className="components-icon-button-text">
						{__("Add ingredient", "delicious-recipes")}
					</span>
				</Button>
				<Button
				...
				>
					<span className="components-icon-button-text">
						{__("Add ingredient section", "delicious-recipes")}
					</span>
				</Button>
			</div>
		);
	...
	render() {
		...
		const classNames = ["recipe-card-ingredients"]
			.filter((item) => item)
			.join(" ");
		const listClassNames = ["ingredients-list", `layout`]
			.filter((item) => item)
			.join(" ");
		...
		return (
			<div className={classNames}>
				<RichText
					tagName="h3"
					className="ingredients-title dr-title"
					format="string"
